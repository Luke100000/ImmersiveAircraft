...
public abstract class DyeableVehicleEntityRenderer<T extends DyeableVehicleEntity> extends VehicleEntityRenderer<T> {
    ...
    public void renderUndyed(BBModel model, BBObject object, SubmitNodeCollector collector, T entity, PoseStack matrixStack, int light, float time) {
        if (entity.getDyeColor() < 0) {
            renderObjectInner(model, object, matrixStack, collector, light, time, entity,  null,  1.0f,1.0f,1.0f, 1.0f) ;
        }
    }

    public void renderDyed(BBModel model, BBObject object, SubmitNodeCollector collector, T entity, PoseStack matrixStack, int light, float time, boolean highlight, boolean hideWhenUndyed) {
        ...
        renderObjectInner(model, object, matrixStack, collector, light, time, entity,  null,  rgb[0], rgb[1], rgb[2], 1.0f) ;
    }
}
