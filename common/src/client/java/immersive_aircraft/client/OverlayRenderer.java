...
public class OverlayRenderer {
    ...
    private void renderAircraftHealth(Minecraft minecraft, GuiGraphics context, VehicleEntity vehicle, int barHeightOffset) {
        ...
        for (int i = 0; i < maxHearts; i++) {
            int u = 52;
            int x = ox - i * 8 - 9;
            blit(context, ICONS_TEX, x, y, u, 9, 9, 9, 64, 64);
            if (i * 2 + 1 < health) {
                blit(context, ICONS_TEX, x, y, 0, 0, 9, 9, 64, 64);
            }
            if (i * 2 + 1 != health) continue;
            blit(context, ICONS_TEX, x, y, 10, 0, 9, 9, 64, 64);
        }
    }
    ...
    private void renderAircraftGui(Minecraft client, GuiGraphics context, float tickDelta, EngineVehicle aircraft) {
        ...
        if (aircraft.getGuiStyle() == EngineVehicle.GUI_STYLE.ENGINE) {
            ...
            blit(context, ENGINE_TEX, x - 9, y - 9, (frame % 5) * 18, Math.floorDiv(frame, 5) * 18, 18, 18, 90, 90);

            blit(context, POWER_TEX, x - 9, y - 9, (powerFrame % 5) * 18, Math.floorDiv(powerFrame, 5) * 18, 18, 18, 90, 90);
            blit(context, POWER_TEX, x - 9, y - 9, (powerFrameTarget % 5) * 18, Math.floorDiv(powerFrameTarget, 5) * 18, 18, 18, 90, 90, ARGB.white(0.5f));
        }
    }
    ...
    private static void blit(GuiGraphics context, Identifier texture, int x, int y, int u, int v, int w, int h, int texW, int texH) {
        context.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, (float) u, (float) v, w, h, texW, texH);
    }

    private static void blit(GuiGraphics context, Identifier texture, int x, int y, int u, int v, int w, int h, int texW, int texH, int color) {
        context.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, (float) u, (float) v, w, h, texW, texH, color);
    }
}
