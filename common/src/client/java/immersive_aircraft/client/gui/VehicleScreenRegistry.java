...
public class VehicleScreenRegistry {
    ...
    public static final OpenGuiRequestHandler DEFAULT = (vehicle, player, message) -> {
        ...
        if (client.level != null && player != null) {
            VehicleScreenHandler handler = (VehicleScreenHandler) vehicle.createMenu(message.getSyncId(), player.getInventory(), player);
            VehicleScreen screen = new VehicleScreen(handler, player.getInventory(), vehicle.getDisplayName());
